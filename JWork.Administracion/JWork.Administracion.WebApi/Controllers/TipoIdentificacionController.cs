...
namespace JWork.Administracion.WebApi.Controllers
{
    ...
    public class TipoIdentificacionController : ControllerBase
    {
        ...
        public async Task<ActionResult<Response<List<TipoIdentificacionDto>>>> BuscarPaginado([FromQuery] Buscar.TipoIdentificacionPaginadoBuscarCommand command)
        {
            try
            {
                List<TipoIdentificacionDto> actividadDtos = await _mediator.Send(command);

                var response = new Response<List<TipoIdentificacionDto>>
                {
