...
namespace JWork.Administracion.WebApi.Controllers
{
    ...
    public class ConceptoCalificacionController : ControllerBase
    {
        ...
        public async Task<ActionResult<Response<List<ConceptoCalificacionDto>>>> BuscarPaginado([FromQuery] Buscar.ConceptoCalificacionPaginadoBuscarCommand command)
        {
            try
            {
                List<ConceptoCalificacionDto> actividadDtos = await _mediator.Send(command);

                var response = new Response<List<ConceptoCalificacionDto>>
                {
