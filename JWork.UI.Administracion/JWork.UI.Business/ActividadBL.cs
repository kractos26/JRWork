...
namespace JWork.UI.Administracion.Business
{
    public class ActividadBL
    {
        ...
        public async Task<Response<List<ActividadDto>>> GetTodoAsync() => await _service.BuscarTodoAsync();
        public async Task<Response<ActividadDto>> GetPorIdAsync(int id) => await _service.BuscarPorIdAsync(id);
        public async Task<Response<List<ActividadDto>>> Buscar(ActividadRequest  request) => await _service.Buscar(request);
